...
def ctsegNegativeControlRadiomicFeatureExtractionParallel(summaryFilePath:str,
                                                        summaryJSONPath: str,
                                                        pyradiomicsParamFilePath: str,
                                                        idColumnName:str,
                                                        imageDirPath: str,
                                                        segmentationDirPath: str,
                                                        segmentationLabel: int = 1,
                                                        roiNames: str = None,
    ...
    Returns
    -------
    Dataframes containing extracted PyRadiomics features for each sample in summaryFilePath and a corresponding negative control sample.
    '''
    # TODO: figure out what to set as default pyrad parameters
        ...
        for segCount, segSeries in enumerate(segSeriesIDs):
            ...
            if not segImages:
                ...
                for roiCount, roi in enumerate(segImages):
                    ...
                    alignedROIImage = alignImages(ncCTImage, roiImage)
                    ...
                    try:
                        ...
                        if ncCTImage.GetSize() != alignedROIImage.GetSize():
                            # Checking if number of segmentation slices is less than CT 
                            if ncCTImage.GetSize()[2] > alignedROIImage.GetSize()[2]:  
                                print("Slice number mismatch between CT and segmentation for", patID, ". Padding segmentation to match.")
                                alignedROIImage = padSEGtoMatchCT(ctFolderPath, segFilePath, ncCTImage, alignedROIImage)
                            else:
                                raise RuntimeError()

                        # Check that image and segmentation mask correspond
                        segBoundingBox, correctedROIImage = imageoperations.checkMask(ncCTImage, alignedROIImage, label=segmentationLabel)
                        ...
                        croppedImage, croppedROI = imageoperations.cropToTumorMask(ncCTImage, alignedROIImage, segBoundingBox)
                    
                        # Extract features listed in the parameter file
                        print('Calculating features for patient:', patID, ", segmentation:", roi)

                        # Extract radiomic features from CT with segmentation as mask, with segmentation pixels set to 1 by default
                        ...
                        # Store this ROI's info in the segmentation level dictionary
                        ncAllData.append(sampleROIData)

                    except Exception as e:
                        print(str(e))

        return ncAllData
    # END FEATUREEXTRACTION INNER FUNCTION
